 ...
 * 		hash:7c6acabc99609dd1e2a5, chunkhash:38a220d3c206f144a4c3, name:auth, filebase:auth.0.0.1902-2.en.js, query:, file:auth.0.0.1902-2.en.js
 */
...
"use strict";
Object.defineProperty(__webpack_exports__, "__esModule", { value: true });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__style_styl__ = __webpack_require__(81);
...
function mount(content) {
	__WEBPACK_IMPORTED_MODULE_1_riot__["mount"](document.getElementById('app').appendChild(content));
}
...
/***/ (function(module, exports, __webpack_require__) {
    ...
    var riot = __webpack_require__(0)
    riot.tag2('mk-form', '<header><h1><i>{app.name}</i>があなたの<b>アカウント</b>に<b>アクセス</b>することを<b>許可</b>しますか？</h1><img riot-src="{app.icon_url + \'?thumbnail&size=64\'}"></header><div class="app"><section><h2>{app.name}</h2><p class="nid">{app.name_id}</p><p class="description">{app.description}</p></section><section><h2>このアプリは次の権限を要求しています:</h2><ul><virtual each="{p in app.permission}"><li if="{p == \'account-read\'}">アカウントの情報を見る。</li><li if="{p == \'account-write\'}">アカウントの情報を操作する。</li><li if="{p == \'post-write\'}">投稿する。</li><li if="{p == \'like-write\'}">いいねしたりいいね解除する。</li><li if="{p == \'following-write\'}">フォローしたりフォロー解除する。</li><li if="{p == \'drive-read\'}">ドライブを見る。</li><li if="{p == \'drive-write\'}">ドライブを操作する。</li><li if="{p == \'notification-read\'}">通知を見る。</li><li if="{p == \'notification-write\'}">通知を操作する。</li></virtual></ul></section></div><div class="action"><button onclick="{cancel}">キャンセル</button><button onclick="{accept}">アクセスを許可</button></div>', 'mk-form,[data-is="mk-form"]{display:block;} mk-form > header > h1,[data-is="mk-form"] > header > h1{margin:0;padding:32px 32px 20px 32px;font-size:24px;font-weight:normal;color:#777;} mk-form > header > h1 i,[data-is="mk-form"] > header > h1 i{color:#77aeca;} mk-form > header > h1 i:before,[data-is="mk-form"] > header > h1 i:before{content:\'「\'} mk-form > header > h1 i:after,[data-is="mk-form"] > header > h1 i:after{content:\'」\'} mk-form > header > h1 b,[data-is="mk-form"] > header > h1 b{color:#666} mk-form > header > img,[data-is="mk-form"] > header > img{display:block;z-index:1;width:84px;height:84px;margin:0 auto -38px auto;border:solid 5px #fff;border-radius:100%;box-shadow:0 2px 2px rgba(0,0,0,0.1)} mk-form > .app,[data-is="mk-form"] > .app{padding:44px 16px 0 16px;color:#555;background:#eee;box-shadow:0 2px 2px rgba(0,0,0,0.1) inset;} mk-form > .app:after,[data-is="mk-form"] > .app:after{content:\'\';display:block;clear:both} mk-form > .app > section,[data-is="mk-form"] > .app > section{float:left;width:50%;padding:8px;text-align:left;} mk-form > .app > section > h2,[data-is="mk-form"] > .app > section > h2{margin:0;font-size:16px;color:#777} mk-form > .action,[data-is="mk-form"] > .action{padding:16px;} mk-form > .action > button,[data-is="mk-form"] > .action > button{margin:0 8px}@media (max-width:600px){ mk-form > header > img,[data-is="mk-form"] > header > img{box-shadow:none} mk-form > .app,[data-is="mk-form"] > .app{box-shadow:none}}@media (max-width:500px){ mk-form > header > h1,[data-is="mk-form"] > header > h1{font-size:16px}}', '', function(opts) {
		this.mixin('api');
...
/***/ (function(module, exports, __webpack_require__) {
    ...
    var riot = __webpack_require__(0)
    riot.tag2('mk-index', '<main if="{SIGNIN}"><p class="fetching" if="{fetching}">読み込み中 <mk-ellipsis></mk-ellipsis></p><mk-form ref="form" if="{state == \'waiting\'}" session="{session}"></mk-form><div class="denied" if="{state == \'denied\'}"><h1>アプリケーションの連携をキャンセルしました。</h1><p>このアプリがあなたのアカウントにアクセスすることはありません。</p></div><div class="accepted" if="{state == \'accepted\'}"><h1>{session.app.is_authorized ? \'このアプリは既に連携済みです\' : \'アプリケーションの連携を許可しました\'}</h1><p if="{session.app.callback_url}">アプリケーションに戻っています <mk-ellipsis></mk-ellipsis></p><p if="{!session.app.callback_url}">アプリケーションに戻って、やっていってください。</p></div><div class="error" if="{state == \'fetch-session-error\'}"><p>セッションが存在しません。</p></div></main><main class="signin" if="{!SIGNIN}"><h1>サインインしてください</h1><mk-signin></mk-signin></main><footer><img src="/assets/auth/logo.svg" alt="Misskey"></footer>', 'mk-index,[data-is="mk-index"]{display:block;} mk-index > main,[data-is="mk-index"] > main{width:100%;max-width:500px;margin:0 auto;text-align:center;background:#fff;box-shadow:0 4px 16px rgba(0,0,0,0.2);} mk-index > main > .fetching,[data-is="mk-index"] > main > .fetching{margin:0;padding:32px;color:#555} mk-index > main > div,[data-is="mk-index"] > main > div{padding:64px;} mk-index > main > div > h1,[data-is="mk-index"] > main > div > h1{margin:0 0 8px 0;padding:0;font-size:20px;font-weight:normal} mk-index > main > div > p,[data-is="mk-index"] > main > div > p{margin:0;color:#555} mk-index > main > div.denied > h1,[data-is="mk-index"] > main > div.denied > h1{color:#e65050} mk-index > main > div.accepted > h1,[data-is="mk-index"] > main > div.accepted > h1{color:#50bbe6} mk-index > main.signin,[data-is="mk-index"] > main.signin{padding:32px 32px 16px 32px;} mk-index > main.signin > h1,[data-is="mk-index"] > main.signin > h1{margin:0 0 22px 0;padding:0;font-size:20px;font-weight:normal;color:#555}@media (max-width:600px){ mk-index > main,[data-is="mk-index"] > main{max-width:none;box-shadow:none}}@media (max-width:500px){ mk-index > main > div > h1,[data-is="mk-index"] > main > div > h1{font-size:16px}} mk-index > footer > img,[data-is="mk-index"] > footer > img{display:block;width:64px;height:64px;margin:0 auto}', '', function(opts) {
		this.mixin('i');
		this.mixin('api');
